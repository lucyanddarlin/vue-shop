				...
				<div class="InfoWrap">
					...
					<div class="choose">
						<div class="chooseArea">
							...
							<dl
								v-for="supAttr in spuSaleAttrList"
								:key="supAttr.id"
							>
								<dt class="title">
									{{ supAttr.saleAttrName }}
								</dt>
								<dd
									changepirce="0"
									:class="{
										active: supAttrValue.isChecked == 1,
									}"
									v-for="supAttrValue in supAttr.spuSaleAttrValueList"
									:key="supAttrValue.id"
									@click="
										changeActive(
											supAttrValue,
											supAttr.spuSaleAttrValueList
										)
									"
								>
									{{ supAttrValue.saleAttrValueName }}
								</dd>
							</dl>
						</div>
						<div class="cartWrap">
							...
							<div class="add">
								<a @click="addCart" href="javascript:"
									>加入购物车</a
								>
							</div>
						</div>
...
export default {
	...
	mounted() {
		this.$store.dispatch('getGoodInfo', this.$route.params.goodsId)
	},
	computed: {
		...mapGetters(['categoryView', 'skuInfo', 'spuSaleAttrList']),
	},
	methods: {
		changeActive(spuAttr, spuAttrList) {
